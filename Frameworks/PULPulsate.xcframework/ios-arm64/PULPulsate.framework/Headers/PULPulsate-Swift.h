...
/// Creates access token from authorization data. If authorization data is nil, this constructor returns nil.
...
- (void)injectData:(id _Nullable)data isInApp:(BOOL)isInApp;
@end
...
@property (nonatomic, copy) NSString * _Nullable btnColor;
@property (nonatomic, copy) NSString * _Nullable txtColor;
...
@interface PULDebugLog (SWIFT_EXTENSION(PULPulsate))
@property (nonatomic, copy) NSString * _Nullable message;
...
- (void)getBadgeCount;
- (void)saveUnauthorizedCampaignWithGUID:(NSString * _Nullable)guid andTitle:(NSString * _Nullable)title andNotificationMessage:(NSString * _Nullable)notificationMessage;
...
- (void)setCustomColorWithThemeKey:(PULThemeKey * _Nonnull)key color:(UIColor * _Nonnull)color;
/// Sets a color for a specific configuration key using a string key
- (void)setCustomColorForKey:(NSString * _Nonnull)key color:(UIColor * _Nonnull)color;
...
/// Gets device information with real-time accurate push permissions (UI-safe)
/// Returns instantly with automatically maintained up-to-date permission status<br/>
/// Push permissions are kept fresh by intelligent background updates
/// \param withPermissions Whether to include permission data
///
///
/// returns:
/// PULDevice with current, accurate permission information
